...
#print(f"dlib 是否使用 CUDA: {dlib.DLIB_USE_CUDA}")
import numpy as np
import os
import logging
...
def preprocess_face_frame(face_img):
    ...
    tensor = transform(face_resized)# 转为张量并归一化（符合模型输入要求）
    return tensor
...
class FaceRecognizerOT:
    ...
    # 相似度计算 euclidean_distance
    def euclidean_distance(self, f1, f2):
        return np.linalg.norm(f1 - f2)
    # 人脸识别 recognize_face
    def recognize_face(self, img, rect):
        # 用predictor提取人脸特征点，用face_reco_model计算128维特征向量
        # 与已知特征库比较，找到最相似的人脸，距离小于0.4则匹配成功，返回姓名；否则返回"unknown"
