...
class FaceRecognizerOT:
    ...
    def warning_worker(self):
        while True:
            try:
                camera_id, warning_type = self.warning_queue.get()
                ...
                try:
                    self.cursor.execute(sql, (camera_id, now, warning_type, video_url, 1))
                    self.conn.commit()
                    logging.info(f"[写入告警] 类型: {warning_type}, 摄像头: {camera_id}")
    ...
    # 修改为异步写入：入队列
    def insert_warning(self, camera_id, warning_type):
        self.warning_queue.put((camera_id, warning_type))
    ...
    def process_single_frame(self, img, stream_url):
        ...
        row_ind, col_ind = linear_sum_assignment(cost_matrix) if cost_matrix and cost_matrix[0] else ([], [])
            ...
            if cost_matrix[r][c] < self.max_dist_for_tracking:
                ...
                if not is_live:
                    ...
                    if t['spoof_cnt'] >= 3 and not t.get('alerted'):
                        self.insert_warning(camera_id, 'cheat')
                        t['alert_type'] = 'cheat'
                        t['alerted'] = True
                else:
                    t['spoof_cnt'] = 0
                    if name == 'unknown':
                        ...
                        if t['unknown_cnt'] >= 3 and not t.get('alerted'):
                            self.insert_warning(camera_id, 'stranger')
        ...
        for t in self.tracked_faces:
            ...
            label = f"{t['name']}[欺骗]" if t['alert_type'] == 'cheat' else t['name']
...
@app.route('/api/video_feed/<stream_id>')
