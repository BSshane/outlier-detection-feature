...
class Face_Recognizer:
    ...
    def draw_chinese_name(self, img_rd, name_list, position_list):
        img_pil = Image.fromarray(cv2.cvtColor(img_rd, cv2.COLOR_BGR2RGB))
        draw = ImageDraw.Draw(img_pil)
        font_chinese = ImageFont.truetype("simsun.ttc", 30)  # 确保你的目录下有这个字体文件
        for name, pos in zip(name_list, position_list):
            # PIL绘制时坐标需是整数元组
            draw.text((int(pos[0]), int(pos[1])), name, font=font_chinese, fill=(255, 255, 0))
    ...
    def process(self, stream):
        ...
        if self.get_face_database():
            while stream.isOpened():
                ...
                for face in faces:
                    cv2.rectangle(img_rd, (face.left(), face.top()), (face.right(), face.bottom()), (255, 0, 0), 2)
                ...
                if (self.current_frame_face_cnt == self.last_frame_face_cnt) and (
                    ...
                    if self.current_frame_face_cnt != 0:
                        for k, d in enumerate(faces):
                            ...
                            name = str(self.current_frame_face_name_list[k])
                            position = self.current_frame_face_position_list[k]
                            img_rd = self.draw_chinese_name(img_rd, self.current_frame_face_name_list, self.current_frame_face_position_list)

                    # 如果当前帧中有多个人脸, 使用质心追踪 / Multi-faces in current frame, use centroid-tracker to track
                    if self.current_frame_face_cnt != 1:
                        self.centroid_tracker()

                    length = min(len(self.current_frame_face_name_list), len(self.current_frame_face_position_list))
                    for i in range(length):
                        name = str(self.current_frame_face_name_list[i])
                        pos = self.current_frame_face_position_list[i]
                        img_rd = self.draw_chinese_name(img_rd, self.current_frame_face_name_list, self.current_frame_face_position_list)
