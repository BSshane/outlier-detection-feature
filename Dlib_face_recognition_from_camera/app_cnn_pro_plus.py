...
def generate_video_stream(stream_url):
    ...
    while True:
        ...
        frame_count += 1
        if frame_count % 5 == 0:
            frame = recognizer.process_single_frame(frame)
        else:
...
def index():
    return """
    <h1>AI Video Service (B) is running.</h1>
    <p>Access a stream via /video_feed/&lt;stream_id&gt;</p>
    <p>Example: <a href="/video_feed/test">/video_feed/test</a></p>
    """
...
def video_feed(stream_id):
    ...
    cap = cv2.VideoCapture(stream_url)
    if not cap.isOpened():
        logging.error(f"Cannot open RTMP stream: {stream_url}")
        return f"无法打开视频流: {stream_url}", 500
    cap.release()
...
# def main():
#    parser=argparse.ArgumentParser(description="实施人脸识别与活体检测")
#    parser.add_argument('--stream-url',type=str,default='0',help="视频流地址")
#    parser.add_argument('--host',type=str,default='0.0.0.0',help="服务器监听主机地址")
#    parser.add_argument('--port',type=int,default=5000)
#    parser.add_argument('--process-interval',type=int,default=5,help="每隔N帧进行完整识别")
#    args = parser.parse_args()
#    # 将解析到的参数存储到 Flask 的配置中，方便路由函数访问
#    app.config['STREAM_URL'] = args.stream_url
#    app.config['PROCESS_INTERVAL'] = args.process_interval
#
#    logging.info(f"启动服务器，监听地址: http://{args.host}:{args.port}")
#    logging.info(f"处理视频源: {args.stream_url}")
#    logging.info(f"处理间隔: 每 {args.process_interval} 帧处理一次")
#    logging.info("在浏览器中打开主页即可看到视频流。")
#
#    # 启动 Flask 服务器
#    # 使用 threaded=True 允许多个客户端同时访问，尽管对于视频流这影响不大
